...
public class Messages {
    ...
    public static final Text TOO_FEW_SELECTIONS = PREFIX.copy().append(new LiteralText("At least one selection is required!"));
    ...
    public static final Text[] GET_STARTED = new MutableText[]{
            ...
            header("How to Claim (Renting)")
                    .append(line("Renting allows you to earn items by selling claim areas to others for a certain amount of time").formatted(Formatting.LIGHT_PURPLE))
                    .append(line(1, "To enable rents in your claim, type ").append(text("/claim rentable <claim> <item> <amount> <rent> <maxrent>").formatted(Formatting.GOLD)))
                    .append(line(2, "eg: ").append(text("/claim rentable shop diamond 14 7d 28d").formatted(Formatting.GOLD)))
                    .append(line(3, "Done! Your claim can now be rented!"))
                    .append(line(4, "If you want to rent a rent, type /claim rent <claim> <rent>"))
    };
    ...
    public static final Text[] SETTINGS_AND_PERMISSIONS = new Text[]{
            header("Permissions")
                    ...
                    .append(line("&6break.[<block>] ").append(text("&eAllows others to break blocks")))
                    .append(line("&6place.[<block>] ").append(text("&eAllows others to place blocks")))
                    .append(line("&6interact_blocks.[<block_entity>] ").append(text("&eAllows others to Interact (Right click) with blocks")))
                    .append(line("&6use_item.[<item>] ").append(text("&eAllows others to use item (Rightclick)"))),
...
//                    .append(line("&6interact_doors ").append(text("&eAllows others to Interact with Doors")))
            ...
            header("Permissions")
                    .append(line("&6damage_entity.[<entity>] ").append(text("&eAllows others to Damage (Attack) Entities")))
                    .append(line("&6interact_entity.[<entity>] ").append(text("&eAllows others to Interact with Entities (Right click on Entities)")))
...
//                    .append(line("&6damage_entity.passive ").append(text("&eAllows others to Damage (Attack) Passive Entities, e.g: Sheep, Cow, Chicken etc")))
                    .append(line("&6modify.[<type>] ").append(text("&eAllows others to modify certain properties of your claim (Use with care)")))
...
/*        public static Text getPermissions(Claim claim) {
            ...
            for (Permission permission : Permission.values()) {
                if (claim.permissionManager.defaults.isPermissionSet(permission.id)) {
                    boolean enabled = claim.permissionManager.defaults.hasPermission(permission.id);
                    if (enabled) {
                    ...
                    claim.permissionManager.defaults.getPermissionList().forEach((perm, value) -> {
